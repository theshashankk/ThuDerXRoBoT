...
async def amireallyalive(alive):
    ...
    if ALV_PIC:
        tele = f"**Welcome To Thunder X **\n\n"
        tele += f"`{CUSTOM_ALIVE}`\n\n"
        tele += (
            f"{telemoji} **Telethon version**: `1.17`\n{telemoji} **Python**: `3.8.3`\n"
        )
        tele += f"{telemoji} **Thunder X Version**: `{telever}`\n"
        tele += f"{telemoji} **More Info**: @Theshashank\n"
        tele += f"{telemoji} **Sudo** : `{sudo}`\n"
        tele += f"{telemoji} **Thunder X Uptime**: `{uptime}`\n"
        tele += f"{telemoji} **Database Status**: `All OK 👌!`\n"
        tele += (
            f"{telemoji} **My pro owner** : [{DEFAULTUSER}](tg://user?id={myid})\n\n"
        )
        tele += "    [✨ GitHub Repository ✨](https://github.com/theshashankk/ThunderXRoBoT)"
        ...
        await alive.delete()
        return
    req = requests.get("https://telegra.ph/file/f8b50242a76cf5db2d10d.jpg")
    ...
    with BytesIO() as sticker:
        ...
        await borg.send_message(
            alive.chat_id,
            f"**Welcome To Thunder X **\n\n"
            f"`{CUSTOM_ALIVE}`\n\n"
            f"{telemoji} **Telethon version**: `1.17`\n{telemoji} **Python**: `3.8.3`\n"
            f"{telemoji} **Thunder X Version**: `{telever}`\n"
            f"{telemoji} **More Info**: @Theshashank\n"
            f"{telemoji} **Sudo** : `{sudo}`\n"
            f"{telemoji} **TeleBot Uptime**: `{uptime}`\n"
            f"{telemoji} **Database Status**: `All OK 👌!`\n"
            f"{telemoji} **My pro owner** : [{DEFAULTUSER}](tg://user?id={myid})\n\n"
            "    [✨ GitHub Repository ✨](https://github.com/theshashankk/ThunderXRoBoT)",
            link_preview=False,
        )
